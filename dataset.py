...
class VOCDataset(Dataset):
    ...
    def __init__(self, root_path, data_name, transforms = None, txt_name = "train.txt"):
        """
        args:
        ...
        self.txt_path = os.path.join(self.voc_path, "ImageSets", "Main", txt_name)
        self.JPG_path = os.path.join(self.voc_path, "JPEGImages")
        self.anno_path = os.path.join(self.voc_path, "Annotations")
        
        # 判断train.txt文件是否存在
        assert os.path.exists(self.txt_path), f"path:{self.txt_path} does not exist"
        with open(self.txt_path, "r") as read:
            self.xml_list = [os.path.join(self.anno_path, line.strip()+".xml") 
