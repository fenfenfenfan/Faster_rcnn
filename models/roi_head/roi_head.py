...
from ops import det_utils
from ops import boxes as box_opts
...
class ROIHeads(torch.nn.Module):
    ...
    def __init__(self, box_roi_pool, box_head, box_predictor, fg_iou_thresh,
                 bg_iou_thresh, batch_size_per_image, positive_fraction,
                 bbox_reg_weights, score_thresh, nms_thresh,
                 detection_per_img):
        super(ROIHeads, self).__init__()
        self.box_similarity = box_opts.box_iou
        self.proposal_matcher = det_utils.Matcher(
            fg_iou_thresh, bg_iou_thresh, allow_low_quaility_matches=False)
        ...
        self.detection_per_image = detection_per_img
    ...
    def forward(
        ...
        if targets is not None:
            for t in targets:
                ...
                assert t[
                    "lables"].dtype == torch.int64, "targets labels must of int64 type"
        if self.training:
    ...
    def postprocess_detections(
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list,
            ...
            boxes = box_opts.clip_boxes_to_image(boxes, image_shape)
            ...
            keep = box_opts.batched_nms(boxes, scores, labels, self.nms_thresh)
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(
            ...
            if gt_boxes_in_image.nueml() == 0:
                ...
                match_quality_matrix = box_opts.box_iou(
                ...
                ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLD
    ...
    def select_training_samples(
        ...
        gt_boxes = [t["boxes"].to(type) for t in targets]
        gt_labels = [t["labels"].to(type) for t in targets]
        ...
        for img_id in range(num_images):
            ...
            if gt_boxes_in_image.nueml() == 0:
                gt_boxes_in_image = torch.zero((1, 4),
                                               dtype=dtype,
                                               device=device)
